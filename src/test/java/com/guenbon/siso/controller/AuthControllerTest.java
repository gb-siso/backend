...
class AuthControllerTest {
    ...
    @DisplayName("카카오 로그인 시 authenticateWithNaver 메서드에서 ApiException이 발생할 시 ApiErrorResponse로 응답한다")
    @ParameterizedTest
    @EnumSource(NaverApiErrorCode.class)
    void naverLogin_ApiExceptionAtAuthenticateWithNaver_ApiErrorResponse(NaverApiErrorCode naverApiErrorCode) throws Exception {
        ...
        when(authApiService.authenticateWithNaver(invalidNaverAuthCode)).thenThrow(new ApiException(naverApiErrorCode));

        // when then
        mockMvc.perform(MockMvcRequestBuilders.get(BASE_URL + LOGIN_NAVER_PATH)
                        .param(CODE, invalidNaverAuthCode) // errorCode 대신 getCode() 사용
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().is(naverApiErrorCode.getHttpStatus().value())) // HTTP 상태 코드 비교
                .andExpect(content().contentType("application/json"))
    ...
    void naverLogin_success_LoginDTO() throws Exception {
        ...
        when(authApiService.authenticateWithKakao(validNaverAuthCode)).thenReturn(expected);
